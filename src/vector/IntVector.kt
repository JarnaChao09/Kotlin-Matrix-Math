...
class IntVector(length: Int = 10, initBlock: (Int) -> Int = { 0 }): NumberVector<Int>(length, initBlock) {
    ...
    sealed class Scope {
        ...
        class Base: IntVector.Scope() /** DO NOT MAKE SINGLETON, EACH MUST HAVE SEPARATE SCOPE TO ACCESS SEPARATE ACTIONS LIST **/

        class Error(msg: String = "Illegal Type was found"): Throwable(msg)

        data class Append(val value: Int, var times: Int = 1): IntVector.Scope()

        data class Push(val value: Int, var times: Int = 1): IntVector.Scope()

        data class Put(val value: Int, val at: Int, var times: Int = 1): IntVector.Scope()

        data class Repeat(val times: Int, val repeat: Scope.() -> Scope): IntVector.Scope() {
            fun run(): Scope {
