...
class DoubleVector(length: Int = 10, initBlock: (Int) -> Double = { 0.0 }): NumberVector<Double>(length, initBlock) {
    ...
    sealed class Scope {
        ...
        class Base: DoubleVector.Scope() /** DO NOT MAKE SINGLETON, EACH MUST HAVE SEPARATE SCOPE TO ACCESS SEPARATE ACTIONS LIST **/

        class Error(msg: String = "Illegal Type was found"): Throwable(msg)

        data class Append(val value: Double, var times: Int = 1): DoubleVector.Scope()

        data class Push(val value: Double, var times: Int = 1): DoubleVector.Scope()

        data class Put(val value: Double, val at: Int, var times: Int = 1): DoubleVector.Scope()

        data class Repeat(val times: Int, val repeat: Scope.() -> Scope): DoubleVector.Scope() {
            fun run(): Scope {
